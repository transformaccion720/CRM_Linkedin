...
import { 
  ...
  AlertTriangle, Plus, Flame, Settings, KeyRound, ChevronRight, MessageSquare, UserPlus, Compass
} from 'lucide-react';
...
interface SidebarProps {
  ...
  activeTab?: string;
  onOpenProfile?: () => void;
  onSwitchTab: (tab: 'contactos' | 'segmentos' | 'funnel' | 'objetivos' | 'seguimientos' | 'lead-hunter' | 'mensajeria' | 'recursos' | 'analytics' | 'ejecutivo' | 'configuracion') => void;
...
function SidebarInner({
  ...
  const content = (
    <>
      ...
      {/* 2. Cazador Web Perú (B2B Exploratorio en Google) */}
      <div
        onClick={() => {
          onSwitchTab('lead-hunter');
          if (onCloseMobile) onCloseMobile();
        }}
        className={`flex items-center gap-2.5 px-2.5 py-2 rounded-lg cursor-pointer transition-all mb-1 ${
      ...
      >
        <Compass className="w-4 h-4 shrink-0 text-[#ff6d3b]" />
        <span className="truncate">Cazador Web Perú</span>
        <span className="ml-auto font-mono text-[9px] px-1.5 py-0.5 rounded-full bg-[#ff6d3b]/20 text-[#ff6d3b] font-bold">
          GOOGLE
        </span>
      </div>
